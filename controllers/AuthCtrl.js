...
import sendReponse, { sendBadRequest, sendNotFound } from "./ResponseCtrl.js";
...
export const StaffLogin = async (req, res) => {
  ...
  let match = false;
  if (staff) match = await bcrypt.compare(password, staff.password);
...
export const StaffSignup = async (req, res) => {
  ...
  await mStaff.save().catch((err) => sendBadRequest(err.message,err,res));
  let token = await generateJWT(staff);
  let data = {
    user: mStaff,
    uid: mStaff._id,
