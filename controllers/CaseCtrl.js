...
async function mergeCases(req, res) {
  ...
  const updateResult = await Page.updateMany(
    { caseId: fromCaseId, hospitalId: hospitalId },
    { $set: { caseId: toCaseId } }
  ...
  const updatedCase = await Case.updateOne(
    ...
    sendBadRequest(
      "Failed while updating total pages (" + updatedCount + ")",
      err,
      res
    )
  );
  ...
  if (updatedCase) {
    await Case.deleteOne({ _id: fromCaseId, hospitalId: hospitalId }).catch(
      (err) => sendBadRequest("Failed while deleting old case", err, res)
    );
  }
...
async function submitCase(req, res) {
  ...
  const job = await taskQueues.mainQueue.add(QueueJobNames.generatePdf, {
    caseId: caseId,
  });
  ...
  ).catch((err) => sendBadRequest("Updating Case", err, res));
  ...
  sendReponse(
    true,
    "Prescription is added to queue and will be shared with user once it is ready.",
    response,
    res
  );
}
...
async function generateCasePDF(req, res) {
  ...
  const pages = await Page.find({ caseId }).catch((err) =>
    sendNotFound("Error while fetching pages", err, res)
  );

  const url = await makePdf(pages, 100, 100, null, null).catch((err) =>
    sendNotFound("Error while generating pdf", err, res)
  );
...
async function viewCase(req, res) {
  ...
  let mCase = await Case.findOne({ _id: caseId }).catch((err) =>
    sendNotFound("Error while fetching case", err, res)
  );

  let doctor = await Staff.findOne({ _id: mCase.doctorId }).catch((err) =>
    sendNotFound("Error while fetching doctor", err, res)
  );
  ...
  if (mCase) {
    ...
    sendBadRequest("Case Not Found", "Case Not Found", res);
  }
}
